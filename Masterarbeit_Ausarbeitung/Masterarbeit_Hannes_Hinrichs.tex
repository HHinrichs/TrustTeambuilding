...
%Dieser Effekt kommt nicht nur bei unverwechselbar menschenähnlichen Avataren vor, sondern kann auch bei anderen Avatar-Konditionen auftreten. Solche identitätsbezogenen Avatar-induzierten Effekte, können die \glqq{}Kognitiven-Einstellungen\dq{} (siehe Kapitel \textit{\nameref{Vertrauen}}) und Verhaltensweisen anderer Personen gegenüber, beeinflussen \citep{lok2003effects}.
...
\textit{Kognitives Vertrauen} in Teams lässt sich nur sehr schwierig aufrechterhalten. So könnte es sein, dass ein Team einen hohen kognitiven Vertrauenswert aufweist, dann jedoch mit einem Rückschlag bei der Bearbeitung einer Aufgabe konfrontiert wird und dies eine Verringerung des \textit{kognitiven Vertrauens} zur Folge hat \citep[S.29-31]{mcallister1995affect}.
Demgegenüber wird das \textit{affektive Vertrauen} im Team durch gemeinsame Rückschläge nicht nachhaltig verringert. Es benötigt eine längerfristige emotionale Krise innerhalb des Teams, um eine Verringerung des \textit{affektiven Vertrauens} zu verursachen. Es wird angenommen, dass das \textit{affektive Vertrauen} eine längerfristige und stärkere Bindung als das \textit{kognitive Vertrauen} schafft \citep[S.29-31]{mcallister1995affect}.
...
Bisherige Studien, die einen Zusammenhang zwischen Vertrauen und Teameffektivität untersuchen, haben positive Zusammenhänge \citep{davis2000trusted}, keine Zusammenhänge \citep{hertel2004managing} sowie negative Zusammenhänge \citep{dirks1999effects} festgestellt.
...
%\citep{dirks1999effects} \citep[S.445]{mayer1995integrative}
Mehrdimensionale und zuverlässige Vertrauensforschung ist im traditionellen Sinne nur mittels Langzeitstudien möglich, da das \textit{kognitive} und besonders das \textit{affektive Vertrauen} diese zeitliche Komponente benötigt \citep{jones1998experience}.
...
Schweizer et al. \citep[S.270]{schweitzer2010conceptualizing}\footnote{Die vollständige Liste von Schweizer et al. befindet sich in \textbf{\autoref{criteriaForVirtualTeams}}.} erläutern die 4 größten Kennzeichen eines \textit{virtuellen Teams} in der Literatur.
...
Laut obiger Definition kann fast jedes Team mittlerweile als \textit{virtuelles Team} bezeichnet werden, denn nur selten arbeiten sie ohne Kommunikationstechnologie. 
...
Das \textit{virtuelle Team} zu gründen, stellt nicht die Herausforderung dar. Die eigentliche Herausforderung ergibt sich aus den Unterschiedlichen Kulturen, Entfernungen und Zeitzonen, die ein \textit{virtuelles Team} mitbringt. Wird es geschafft, Vertrauen in das \textit{virtuelle Team} zu bringen, kann der eigentliche Nachteil der verschiedenen Kulturen, Entfernungen und der Zeitzonen auch zum Vorteil werden. Es wird die kulturelle Diversität gefordert und neue Verhaltensmuster erworben, wodurch neue, kreative Sichtweisen gefördert werden. Durch diese Faktoren ist es letztendlich möglich, Innovativer zu Arbeiten und zu denken \citep{dyer1995team} \citep[S.405-416]{milliken1996searching}.
...
%H2$_{0}$ geht davon aus, dass die Kondition, in der eine Person im \ac{sve} die \glqq{}anderen\dq{} teilnehmenden Personen wahrnimmt, ein Signifikant abweichenden Einfluss Somit erscheint ein \ac{vt} in seiner Leistungsfähigkeit und Zuverlässigkeit für eine Person immer gleich, egal ob die Avatare der \glqq{}anderen\dq{} die Kondition \ac{ik} oder \ac{nik} besitzen.
...
%
%Es gibt keinen Signifikanten unterschied in der stärke des Zusammenhangs der CT Werte und Teameffektivität zwischen den Teams der verschiedenen Konditionen.
%
%Korrelation IK, NIK -> Fishers Z Transformation -> Konfidenzintervalle vergleichen; bei überschneidung nicht signifikanter unterschied
%Fishers Z-Transformation : 
%https://blogs.gwu.edu/weissba/teaching/calculators/fishers-z-transformation/
...
%
%Hypothese für zweifaktorieller Varianzanalyse
%%Es gibt einen Unterschied zwischen Teams mit hohen CT-Score und niedrigem CT-Score bei der Teameffektivität in Abhängigkeit der Kondition IK und NIK.
%
...
%Da die teilnehmenden Personen entweder die Avatarkondition \ac{ik} oder die Avatarkondition \ac{nik} während des Versuchs als die \glqq{}anderen\dq{} Personen im Raum wahrnehmen, wird der unterschied der wahrgenommenen Co-Präsenz durch die Avatarkonditionen \ac{ik} und \ac{nik} analysiert.
%
...
%% VGL : https://www.youtube.com/watch?v=SVuniaJXOoM&ab_channel=drfg2008
%
%Da die teilnehmenden Personen entweder die Avatarkondition \ac{ik} oder die Avatarkondition \ac{nik} während des Versuchs als die \glqq{}anderen\dq{} Personen im Raum wahrnehmen, wird der Zusammenhang an gebildeter Co-Präsenz und gebildetem kognitiven Vertrauen zwischen den unterschiedlichen Avatarkonditionen analysiert.
%
%Es wird die \textbf{\textit{Hypothese 6}} aufgestellt :
%
...
Das Lowess-Verfahren \footnote{Das Lowess-Verfahren (Locally-Weighted Scatterplot Smoother) ist ein Verfahren, dass mittels einer lokal gewichteten Regressionsfunktion den Zusammenhang zweier oder mehrerer Variablen visuell ermöglicht.} deutet augenscheinlich auf keinen linearen Zusammenhang zwischen dem \textit{generellen Vertrauenswert} der Kondition \ac{ik} und dem \textit{kognitiven Vertrauenswert} der Kondition \ac{ik} sowie dem \textit{generellen Vertrauenswert} der Kondition \ac{nik} und dem \textit{kognitiven Vertrauenswert} der Kondition \ac{nik} hin. Daher wird die \textit{Linearitätsannahme abgelehnt}.
...
Das Lowess-Verfahren deutet augenscheinlich auf \textit{keinen linearen Zusammenhang} zwischen den \textit{kognitiven Vertrauenswerten der Teams} mit der Kondition \ac{ik} und der \textit{Teameffektivität der Teams} mit der Kondition \ac{ik} sowie auf \textit{keinen linearen Zusammenhang} zwischen den \textit{kognitiven Vertrauenswerten der Teams} mit der Kondition \ac{ik} und der \textit{Teameffektivität der Teams} mit der Kondition \ac{nik} hin. Daher wird die \textit{Linearitätsannahme} abgelehnt.
...
\paragraph{Mann-Whitney-U-Test}
Die Verteilungen der \textit{Teameffektivitätswerte} der Kondition \ac{ik} und der \textit{Teameffektivitätswerte} der Kondition \ac{nik} unterscheidet sich laut Kolmogorov-Smirnov ($p = .181 > \alpha = 0.05$) \textbf{nicht signifikant} von einander. Es gab \textbf{keinen signifikanten Unterschied} der Mittelwerte der Teameffektivität zwischen der Kondition \ac{ik} $(\bar{x} = 9,000$) und der Kondition \ac{nik} $(\bar{x} = 9,000)$ mit $U = 103,500, Z = -.377, p = .706 > \alpha = 0.05, r = -,060$. \\
...
Da jedes Teammitglied dieselbe Anzahl an Runden abgeschlossen hat, besitzt diese drei Teammitglieder die Selbe Teameffektivität. Diese \textit{Teameffektivität} der drei Teammitglieder wurde aufsummiert, um die \textit{Teameffektivität eines Teams} zu erhalten.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{r | p{2cm} p{1.5cm} p{3cm} p{1.5cm} p{1.5cm} p{1.5cm} p{1.5cm}} 
		...
		30 & $4,4$ & 2 & 9 & & \\	
	\end{tabularx}
	\caption[Individuelles generelles Vertrauen und erfolgreich abgeschlossene Runden]{Tabelle der individuellen generellen Vertrauenswerte und der erfolgreich abgeschlossenen Runden (\textit{Teameffektivität}) - Individuell und pro Team zusammengefasst. Kondition 1 definiert die Kondition \ac{ik} und Kondition 2 definiert die Kondition \ac{nik}.}
...
Diese beträgt bei der Kondition \ac{ik} im Durchschnitt $\bar{x} = 3.044$ mit einer Standardabweichung von $\sigma = .473$.\newline 
Bei der Kondition \ac{nik} liegt das arithmetische Mittel dieser bei $\bar{x} = 2.900$ mit einer Standardabweichung von $\sigma = .216$. 
...
Weiterhin beträgt diese bei der Kondition \ac{nik} im Mittel $\bar{x} = 5.360$ mit einer Standardabweichung von $\sigma = .797$. 
...
Diese beträgt bei der Kondition\newline \ac{ik} im Mittel $\bar{x} = 4.685$ mit einer Standardabweichung von $\sigma = 1.883$ und
...
Die \textit{empfundene Belastung} ist laut\newline Kolmogoroff-Smirnov-Test mit $p = .337 > \alpha = 0.05$ normalverteilt. 
Sie beträgt \ac{ik} im Mittel $\bar{x} = 8.166$ mit einer Standardabweichung von $\sigma = 1.544$.
...
Bei der Kondition \ac{nik} beträgt diese im Durchschnitt $\bar{x} = 4.48$ mit einer Standardabweichung von $\sigma = .476$. 
...
Die \textit{wahrgenommmene Teameffektivität} ist laut Kolmogoroff-Smirnov-Test mit $p = .065 < \alpha = 0.05$ normalverteilt. 

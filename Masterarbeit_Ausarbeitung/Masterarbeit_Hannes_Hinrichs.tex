...
\textit{Der Grad der Wirksamkeit} kann bei der \textit{Effektivität} somit immer berücksichtigt werden (\cite{eichhorn2005prinzip}).
...
\label{AvatarTrust}
Um den Einfluss des Grades des Realismus unter Avataren zu erforschen, führten Riedl, Mohr und Kenning (\cite{riedl2014trusting}) eine Studie zum Vertrauensaufbau unter Menschen im Vergleich zu Avataren mit menschenähnlichen Gesichtern durch. Sie fanden heraus, dass es Personen leichter fällt einer realen Person, zu vertrauen als einem Avatar mit menschenähnlichem Gesicht. Es wurde der Frontalkortex - die Gehirnregion, die dafür verantwortlich ist, die Gedanken und Gefühle des Gegenübers zu erahnen - bei Interaktionen mit Menschen mehr angeregt als bei Interaktionen mit Avataren mit menschenähnlichen Gesichtern.
Vertrauen zwischen Menschen wird jedoch in der gleichen Geschwindigkeit aufgebaut wie zwischen Menschen und Avataren (\cite{riedl2014trusting}).
...
Es wird der Zusammenhang zwischen dem generellen Vertrauen\newline (\ac{gt}) und dem kognitiven Vertrauen (\ac{ct}) auf \textit{Konditionsebene} analysiert.
...
Es wird der \textit{Zusammenhang} zwischen dem gebildeten kognitiven Vertrauen (\ac{ct}) und der Anzahl der abgeschlossenen Runden (\ac{tRound}) auf \textit{Teamebene} analysiert.

\paragraph{Avatar Embodiment}
Es wird der \textit{Unterschied} zwischen dem gebildeten kognitiven Vertrauen (\ac{ct}) auf \textit{Konditionsebene} analysiert.
...
\label{IKNIK}
Die Avatare sind in dieser Studie als \dq{}IK-Avatar\dq{} und \dq{}Non-IK-Avatar\dq{} (siehe \textit{Abbildung \ref{AvatareAussehen}}) implementiert. Als Grundlage der Visualisierung der Avatare dient eine Studie von Kristoffer Waldow (\cite{waldow2019investigating}),in der der Einfluss des Aussehens von Avataren in einem Augmented-Reality \ac{sve} untersucht wurde.  Für die vorliegende Studie wurde lediglich die Farbe der Avatare von Grün auf Schwarz geändert. 
...
Insgesamt haben 30 Teilnehmer an dem Versuch teilgenommen. Der Mittelwert des Alters der Teilnehmer betrug ($\bar{x}$ = 30,13) mit einer Standartabweichung von ($\sigma$ = 7,44) Jahren. 

\textbf{\textit{Abbildung \ref{teilnehmerAlter}}} zeigt die Alterstruktur der Personen, die an dem Experiment teilgenommen haben. 
\begin{figure}[H]
		\begin{footnotesize}
		\centering
			\includegraphics[scale=0.4]{Abbildungen/Demographie/alter.JPG}	
			\caption[Die Altersstruktur der Versuchsteilnehmer]{Diese Abbildung zeigt die Altersstruktur der Personen, die an dem Experiment teilgenommen haben.}
			\label{teilnehmerAlter}
		\end{footnotesize}
	\end{figure}
	
Von diesen 30 Teilnehmern waren 19(63,3\%) Männlich und 11(36,7\%) Weiblich. 

\textbf{\textit{Abbildung \ref{teilnehmerGeschlecht}}} zeigt die Verteilung des Biologischen Geschlechts der Personen, die an dem Experiment teilgenommen haben.
\begin{figure}[H]
		\begin{footnotesize}
		\centering
			\includegraphics[scale=0.4]{Abbildungen/Demographie/teilnehmerGeschlecht.JPG}
			\caption[Die Verteilung des biologischen Geschlechts der Versuchsteilnehmer]{Diese Abbildung zeigt die Verteilung des biologischen Geschlechts der Personen, die an dem Experiment teilgenommen haben}
			\label{teilnehmerGeschlecht}
		\end{footnotesize}
	\end{figure}
...
\textbf{\textit{Abbildung \ref{teilnehmerBildungsstand}}} zeigt die Verteilung des Biologischen Geschlechts der Personen, die an dem Experiment teilgenommen haben.
\begin{figure}[H]
		\begin{footnotesize}
		\centering
...
\textbf{\textit{Abbildung \ref{teilnehmerVRErfahrungen}}} zeigt die VR Erfahrung sowie die VR-Studien Erfahrung der Teilnehmer.
\begin{figure}[h]
  \centering
...
Die Verteilungsform der beiden Konditionen unterscheiden sich laut\newline Kolmogorov-Smirnov ($p = .181 > \alpha = 0.05$) \textbf{nicht signifikant} voneinander. Es gab \textbf{einen signifikanten Unterschied} der Mittelwerte des \ac{ct} zwischen ($\ac{ik} : \tilde x = 4,1238$) und\newline ($\ac{nik} : \tilde x = 4,5905)$, ($U = 62,500 , Z = -2.083, p = .037 < \alpha = 0.05, r =-.3803$). \\
...
Es ist eine positive Korrelation starken Effektes (vgl. \cite{cohen2013statistical}) mit dem Spearman-Korrelationskoeffizient ($r = .441$) zwischen \textbf{\ac{gti}} und\newline \textbf{\ac{cti}} vorhanden. Die Spearman-Korrelation ist \textbf{nicht signifikant} ($p = .100 > \alpha = 0.05$).\\
Es ist eine positive Korrelation moderaten Effektes (vgl. \cite{cohen2013statistical}) mit dem Spearman-\newline Korrelationskoeffizient ($r = .378$) zwischen \textbf{\ac{gtn}} und \textbf{\ac{ctn}} vorhanden. Die\newline Spearman-Korrelation ist \textbf{nicht signifikant} ($p = .165 > \alpha = 0.05$).
...
Es ist eine positive Korrelation moderaten Effektes (vgl. \cite{cohen2013statistical}) mit dem Spearman-Korrelationskoeffizient ($r = -.205$) zwischen \textbf{\ac{teamCogIK}} und\newline \textbf{\ac{tRoundIK}} vorhanden. Die Spearman-Korrelation ist \textbf{nicht signifikant} ($p = .741 > \alpha = 0.05$).\\
Weiterhin ist eine positive Korrelation moderaten Effektes (vgl. \cite{cohen2013statistical}) mit dem Spearman-Korrelationskoeffizient ($r = .462$) zwischen \textbf{\ac{teamCogNIK}} und \textbf{\ac{tRoundNIK}} vorhanden. Die Spearman-Korrelation ist \textbf{nicht signifikant} ($p = .434 > \alpha = 0.05$).
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{r | p{2cm} p{1.5cm} p{3cm} p{1.5cm} p{1.5cm} p{1.5cm} p{1.5cm} } 
		...
		30 & $3,57$ & 2 & 9 & & \\	
	\end{tabularx}
		\caption[Individuelles kognitives Vertrauen und erfolgreich abgeschlossenen Runden]{Tabelle des kognitiven Vertrauen und der erfolgreich abgeschlossenen Runden - Individuell und pro Team zusammengefasst. Kondition 1 definiert die Kondition \ac{ik} und Kondition 2 definiert die Kondition \ac{nik}.}
...
\paragraph{Spearman-Korrelation}
Es ist eine positive Korrelation schwachen Effektes (vgl. \cite{cohen2013statistical}) mit dem Spearman-Korrelationskoeffizient ($r = .200$) zwischen \textbf{\ac{teamGenIK}} und\newline \textbf{\ac{tRoundIK}} vorhanden. Die Spearman-Korrelation ist \textbf{nicht signifikant} ($p = .747 > \alpha = 0.05$).\\
...
Es wurde auf jedoch Konditionsebene ist zwischen dem kognitiven Vertrauenswert\newline (\ac{ctn}) und der wahrgenommenen Teameffektivität (\ac{ten}) der Kondition\newline \ac{nik} eine \textit{signifikante positive} Korrelation festgestellt. 

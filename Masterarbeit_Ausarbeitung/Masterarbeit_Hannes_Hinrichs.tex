...
Dabei wird der Fokus auf die beiden Konditionen \ac{ik} sowie \ac{nik} gelegt um zu analysieren, bei welcher dieser Konditionen eine größere Effizienzsteigerung innerhalb des Teams vorhanden ist.
Es wird ein spezieller Fokus auf das kognitive Vertrauen in die Teammitglieder, das generelle Vertrauen eines Individuums sowie die daraus resultierende Team-Effizienz gelegt.

	Durch diese Masterarbeit soll es leichter möglich sein, eine Entscheidung über die Wahl und das Design einer Repräsentation eines Avatars in einer Teambuildingmaßnahme zu treffen, um Teambuilding über \ac{vr} effizienter zu gestalten.\\
	Es sind verschiedene Hypothesen aufgestellt, anhand denen es möglich ist, das kognitive Vertrauen in das Team, das generelle Vertrauen eines Individuums, die Teameffizienz und deren Wechselwirkung zu analysieren.
...
Was ist ein Team? Wie unterscheidet sich ein "normales" Team von einem Team, welches räumlich voneinander getrennt ist? "Normale" Teams, haben die Möglichkeit sich jederzeit oder mit einem geringem Zeitaufwand von Angesicht zu Angesicht zu treffen, während räumlich voneinander getrennte Teams nicht diese Möglichkeit besitzen.
		...
		\subsubsection{Definition eines Teams}
...
\subsubsection{Definition eines virtuellen Teams}

https://sci-hub.ren/10.1111/j.1365-2575.2009.00326.x

Virtuelle Teams teilen sehr viele Eigenschaften eines herkömmlichem Team. Als aller erstes ist ein \frqq virtuelles Team\flqq ein \frqq Team\flqq. Anschließend muss unterschieden werden, wie die virtuelle Komponente des Teams definiert werden kann und was diese von einem herkömmlichem Team unterscheidet. 
...
Laut dieser Definition ist jedes Team ein virtuelles Team. Nur selten arbeiten Team während der Lebensdauer dieses nach vorheriger definition. Es wird daher davon ausgegangen, dass Virtualität als Kontinuum gesehen werden kann, bei dem jedes Team einen gewissen Grad an Virtualität besitzt. Dieses Kontinuum reicht von Face-to-Face bis zur vollständigen nur über Kommunikationstechnologie stattfindende Kommunikation \cite{martins2004virtual}.

Der Grad an Virtualität kann gemessen werden. Sehr häufig werden in der Literatur zum messen von Virtualität die Kriterien der Entfernung zwischen den einzelnen Teammitgliedern, das Ausmaß in dem die Teammitglieder Face-to-Face arbeiten, der Anteil der Asynchronen Kommunikation, die räumliche Getrenntheit sowie das Ausmaß in dem die Teammitglieder auf virtuelle Kommunikationstechnologien angewiesen sind, herangezogen.\citep[p.275]{schweitzer2010conceptualizing}

Es jedoch gibt es bisher keine Einigkeit, welche Kriterien zum messen von Virtualität genau herangezogen werden sollten.

In einem Modell von Kirkman lässt sich Virtualität mittels drei Punkten messen. Dem Ausmaß, in dem Teammitglieder auf Virtuelle Tools (z.B. Videochat) angewiesen sind, dem Ausmaß, des übermittelten Informationsgehaltes zum Beitrag von Teameffektivität (z.B. trägt ein Videochat mehr Informationsgehalt bei einer Produktpräsentation bei als ein Telefonat) durch das verwendete Tool und dem Außmaß der zeitlichen Synchronität des gemeinsam Arbeitendem Teams.

In dieser Studie sind die Teams sehr stark auf virtuelle Tools angewiesen, der übermittelte Informationsgehalt zum Beitrag der Teameffektivität ist mittelmäßig, da die Teammitglieder nicht miteinander Sprechen können, jedoch Gestikulieren und die zeitliche Synchonität ist hoch, da alle Teammitglieder gleichzeitig an einer Aufgabe arbeiten.

Somit ist die Virtualität der Teams dieser Studie laut Kirkman mittelmäßig ausgeprägt.
...
Eine idiale Teambuilding Situation ist daher in virtuellen Teams nicht möglich. \textbf{(Cianni andWnuck, 1997) ( Hier einfügen oder doch als eigenen Satz stehenlassen? )}
...
Teameffizienz ist die Fähigkeit eines Teams, so miteinander zu interagieren, dass die Ziele des Teams unterstützt werden. Dies ist wichtig zu beachten, da viele externe Faktoren zum Erfolg oder Teamerfolg beitragen können.
\citep[p.557]{salas2005there}
	...
	Training im Team kann die gesamte Performance eines Teams steigern. Training im Team scheint am effektivsten, wenn mehrere Charakteristika des Teamworks auf einmal angesprochen werden. Diese sollten auch experimentelle Aktivitäten beinhalten um aktiv zu lernen und zu üben. \citep[19]{mcewan2017effectiveness}
...
Der Begriff Vertrauen ist in viele Bereiche des Alltäglichen Lebens eingezogen. 
...
Da diese Arbeit sich mit dem zwischenmenschlichem Vertrauen beschäftigt, wird das Vertrauen in die Akzeptanz nicht weiter behandelt.
Vertrauen wird in dieser Ausarbeitung als bilaterales Konstrukt zwischen einer vertrauenden Person und einer zu vertrauenden Person definiert. \citep[p.728-729]{mayer1995integrative}
...
Aufgrund der Vielseitigkeit von Vertrauen, gibt es unterschiedliche Definitionen. So hat 
 Mayer definiert Vertrauen als :"die Bereitschaft einer Person, für die Handlungen einer anderen Person anfällig zu sein, basierend auf der Erwartung, dass die zu vertrauende Person eine bestimmte, für den Vertrauensgeber wichtige Handlung ausführen wird, unabhängig von der Möglichkeit, diese Person zu überwachen oder zu kontrollieren." \citep[p.712]{mayer1995integrative} \\

Jede zwischenmenschliche Beziehung beginnt mit einer frühen Phase der Vertrauensbildung. Diese frühen Phase kann mit Unsicherheiten und Zweifel charakterisiert werden. Das gegenseitige Vertrauen, dass man sich anschließend schenkt, muss erst einmal ausgelotet werden. \citep[p.166-168]{meyerson1996swift}
...
Auch im Unternehmertum muss über das Konzept des Vertrauens nachgedacht werden um Erfolgreich arbeiten zu können. Ohne Vertrauen in Ihr Team oder in Unterschiedliche Personen, fällt es schwer Risiken einzugehen. Ist Vertrauen vorhanden, wird man nicht mit der Angst konfrontiert, dass andere Personen einen ausnutzen könnten. \citep[p.1152]{breuer2016does}.
...
Diese beiden Dimensionen bestätige McAllister \citep{mcallister1995affect} ebenfalls in seinen Forschungen. Er definierte die beiden Punkte als Kognitives Vertrauen und Affektives Vertrauen.
Affektives Vertrauen ergibt sich somit aus zwischenmenschlichen emotionalen Verbindungen und gegenseitiger Fürsorge, während individuelles kognitives Vertrauen auf der Überzeugung in die Fähigkeiten oder in die Zuverlässigkeit eines anderen basiert. \\
...
\textit{Hypothese H5} : \textbf{Teams, mit einem generell hohem Hang zum Vertrauen erzielen auch eine hohere Team-Effektivität.}
		...
		Warum wurde in dieser Studie Cognitive Vertrauen genutzt?
		...
		Warum wurde in dieser Studie Propensity to Trust  genutzt?
...
\multirow{2}{3.5cm}{Räumliche Präsenz \newline Umschlossenheitsgefühl \newline Erfahrener Realismus}
    ...
    \hline
\multirow{2}{3.5cm}{Co-Präsenz \newline \newline die wahrgenommene Präsenz des \flqq anderen\frqq \newline \newline Telepresence \newline \newline Social-Presence}
...
Insgesamt haben ($N=30$) Personen an dem Versuch teilgenommen. \ac{gt} ist unabhängig vom \ac{gt} der einzelnen Testperson. Daher muss eine Korrelation auf Individuallevel durchgeführt werden, bei der die einzelnen Teams und Teamzusammensetzung ignoriert wird. 
...
Um nun die gerichtete Annahme der positiven Auswirkung des \ac{gt} auf den \ac{ct} zu bestätigen, wurde eine Regressionsanalyse durchgeführt. Wobei wurde als unabhängige Variable \ac{gt} und als abhängige Variable \ac{ct} gewählt.
...
Es wird der Zusammenhang zwischen den unabhängigen Variablen \ac{ik} und \ac{nik} sowie \ac{cti} und \ac{ctn} auf Teamlevel analysiert. Dazu wird ein T-Test durchgeführt. Die Gruppierungsvariablen sind dabei als \ac{ik} und \ac{nik} definiert.
...
Der T-Wert ist kleiner Null ($T = -1,685 < 0$), was auf einen Mittelwertsunterschied zwischen \ac{ct} von \ac{ik} und \ac{nik} zeigt. Der Mittelwert $\bar(x)$ der Kondition \ac{ik} ist kleiner als der Mittelwert der Kondition \ac{nik}.
...
Da die individuellen Vertrauenswerte zusammengefasst wurden, wird der Ausreißer \flqq ID 1\frqq ignoriert und die Kognitiven Vertrauenswerte müssen nicht winsorisiert werden. Alle Ergebnisse werden in \ac{ik} und \ac{nik} aufgeteilt um ein Vergleich der verschiedenen Konditionen darzustellen. Für diesen Vergleich wurde eine Pearson-Korrelationsanalyse durchgeführt.
...
\begin{figure}[H]
		\begin{footnotesize}
			\includegraphics[width=\textwidth]{Abbildungen/criteriaForVirtualTeams.JPG}

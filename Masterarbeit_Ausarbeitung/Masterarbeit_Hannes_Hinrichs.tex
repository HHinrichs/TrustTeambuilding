...
Eine Analyse des kognitiven Vertrauens auf Konditionsebene, soll feststellen, ob sich unterschiedliche Avatarkonditionen auf die Wahrnehmung einer Person, in die Kompetenzen, die Fähigkeiten und die Zuverlässigkeit eines \ac{vt} im \ac{sve}, auswirken.
...
\textbf{H1$_{0}$}: Die Mittelwerte der erzielten kognitiven Vertrauenswerte (\ac{ct}) unterscheiden sich bei den Konditionen \ac{ik} und \ac{nik} nicht signifikant voneinander. \\
...
\textbf{H2$_{0}$}: Es besteht kein Zusammenhang zwischen dem erzielten generellen Vertrauenswert \ac{gt} einer Person und dem erzielten kognitiven Vertrauenswert \ac{ct} einer Person.
...
Ein starkes zwischenmenschliches Vertrauen in traditionell geformten Teams beeinflusst die reale Teameffektivität positiv. Daher wird davon ausgegangen, dass dies auch auf \ac{vts} zutrifft. 
%Es wird das kognitive Vertrauen analysiert, da das affektive Vertrauen vernachlässigt werden kann. 
Je größer das kognitive Vertrauen eines Teams ist, desto mehr können die einzelnen Teammitglieder ihre Ressourcen in die Teamarbeit einbringen. 
...
Das kognitive Vertrauen eines Teams, könnte sich auf die Anzahl erfolgreich abgeschlossenen Runden eines Teams \ac{tRound} auswirken. 
...
\textbf{H3$_{0}$}: Der Zusammenhang zwischen dem kognitiven Vertrauenswert von Teams \ac{teamCog} und der realen Teameffektivität von Teams\newline \ac{tRound} mit der Kondition \ac{ik} unterscheiden sich nicht signifikant von den Teams mit der Kondition \ac{nik}. \\
%
%Was denke ich denn? Ich denke : 
...
Die unterschiedlichen Avatarkonditionen könnten einen Einfluss auf die Anzahl der abgeschlossenen Runden eines Teams - und somit auf die reale Teameffektivität - haben (\textit{siehe Kapitel \ref{Teameffektivität}}). Um dies zu untersuchen, wurde \textbf{\textit{Hypothese H4$_{1}$}} aufgestellt :\\
...
H4$_{0}$: Die Mittelwerte der realen Teameffektivität \ac{tRound} unterscheidet sich bei den Konditionen \ac{ik} und \ac{nik} nicht signifikant voneinander. \\
%%----- ALT
...
Um zu untersuchen, ob die Teameffektivität (\textit{siehe \ref{Teameffektivität}}) im Zusammenhang mit dem generellen Vertrauen steht, wurde \textbf{\textit{Hypothese 5}} aufgestellt:\\
...
\textbf{H5$_{0}$}: Der Zusammenhang zwischen dem generellen Vertrauenswert eines Teams \ac{teamGen} und der realen Teameffektivität eines Teams \ac{tRound} mit der Kondition \ac{ik} unterscheidet sich nicht von den Teams mit der Kondition \ac{nik}. \\
...
Zusätzlich zu den 5 Hypothese, wurde noch der Unterschied zwischen folgenden abhängigen Variablen und der unabhängigen Variable \textit{Avatarkondition} analysiert : Wahrgenommene Teameffektivität (\ac{te}), Presence (\ac{ipq}), Subjektiv wahrgenommene Arbeitsbelastung (\ac{tlx}), Team-Kommunikation (\ac{tc}) sowie Co-Presence( \ac{cp})
...
H1$_{0}$ : Die Mittelwerte der erzielten kognitiven Vertrauenswerte (\ac{ct}) unterscheiden sich bei den Konditionen \ac{ik} und \ac{nik} nicht voneinander.

\paragraph{Allgemeines}
Auf \textit{Konditionsebene} wird analysiert, ob die teilnehmenden Personen aufgrund der unterschiedlichen Avatarkonditionen einen unterschiedlich großen kognitiven Vertrauenswert angeben. Dazu wird der Mann-Whitney-U-Test durchgeführt.\\
Der \ac{ct} ist laut dem Shapiro-Wilk-Test\footnote{Der Shapiro-Wilk-Test ist ein Signifikanztest, der überprüft, ob eine Grundgesamtheit Normalverteilt ist \citep[p.25]{razali2011power}.} mit $p = .023 < \alpha = 0.05$ \textit{nicht normalverteit}.
...
Der Cognitive-Trust-Score der Kondition \ac{ik} (\ac{cti}) beträgt im Mittel $\tilde x = 4,1238$ mit einer Standardabweichung von $\sigma = .67194$.\newline 
Der Cognitive-Trust-Score der Kondition \ac{ik} (\ac{ctn}) beträgt  im Mittel $\tilde x = 4,5905$ mit einer Standardabweichung von $\sigma = .49152$ (siehe \textbf{\textit{Abbildung \ref{H1_Mittelwerte_Auswertung}}}).
...
Die Verteilungsformen der beiden Konditionen unterscheiden sich laut\newline Kolmogorov-Smirnov ($p = .181 > \alpha = 0.05$) \textbf{nicht signifikant} voneinander. Es gab laut Mann-Whitney-U-Test \textbf{einen signifikanten Unterschied} der Mittelwerte des \ac{ct} zwischen den Avatarkonditionen $\ac{ik} (\tilde x = 4,1238)$ und  $\ac{nik} (\tilde x = 4,5905)$ mit $U = 62,500 , Z = -2.083, p = .037 < \alpha = 0.05, r =-.3803$. \\

	\begin{figure}[H]
		\begin{footnotesize}
		\centering
			\includegraphics[scale=0.3]{Abbildungen/AuswertungDiagramme/H1_Mittelwerte.png}
			\caption[Mittelwerte und Standardabweichungen der kognitiven Vertrauenswerte]{Mittelwerte und Standardabweichung der kognitiven Vertrauenswerte. Links die Kondition \ac{ik} und rechts die Kondition \ac{nik}}
...
%H1$_{0}$ : Steigt der General-Trust-Score einer Person (\ac{gt}), steigt der Cognitive-Trust-Score einer Person (\ac{ct}) nicht.
H2$_{0}$ : Es besteht kein Zusammenhang zwischen dem erzielten generellen Vertrauenswert \ac{gt} einer Person und dem erzielten kognitiven Vertrauenswert \ac{ct} einer Person.

\paragraph{Allgemeines}
Der kognitive Vertrauenswert (\ac{ct}) muss für \textit{Hypothese 1} auf \textit{Konditionsebene} analysiert werden, da der \ac{ct} aufgrund der unterschiedlichen Avatarkonditionen gebildet wurde. Somit ist es nicht möglich, den Zusammenhang zwischen \ac{gt} und \ac{ct} auf Individualebene zu betrachten.

Zur Analyse werden die Variablen \textit{\ac{gti}} und \textit{\ac{gtn}} für das generelle Vertrauen der Avatarkondition \ac{ik} sowie die Variablen \textit{\ac{cti}} und \textit{\ac{ctn}}, für das kognitive Vertrauen der Avatarkondition \ac{ik} verwendet.

Der Shapiro-Wilk-Test zeigt, dass das \ac{gti} mit  $p = .078 > \alpha = 0.05$ \textit{normalverteilt} und \ac{gtn} mit $p = .020 < \alpha = 0.05$ \textit{nicht normalverteilt} ist.

Aus einem weiteren Shapiro-Wilk-Test geht hervor, dass \ac{cti} mit  $p = .356 > \alpha = 0.05$ \textit{normalverteilt} und die \ac{ctn} mit $p = .054 > \alpha = 0.05$ \textit{normalverteilt} ist.
Das Lowess-Verfahren \footnote{Das Lowess-Verfahren (LOcally-WEighted Scatterplot Smoother) ist ein Verfahren, dass mittels einer lokal gewichteten Regressionsfunktion den Zusammenhang zweier oder mehrerer Variablen visuell ermöglicht.} deutet Augenscheinlich auf keinen linearen Zusammenhang zwischen \ac{gti} und \ac{cti} sowie \ac{gtn} und \ac{ctn} hin. Daher wird die \textit{Linearitätsannahme nicht angenommen}.
...
Es ist eine positive Korrelation starken Effektes (vgl. \citep{cohen2013statistical}) mit dem Spearman-Korrelationskoeffizient $r = .441$ zwischen \textbf{\ac{gti}} und\newline \textbf{\ac{cti}} vorhanden. Die Spearman-Korrelation ist \textbf{nicht signifikant} ($p = .100 > \alpha = 0.05$).
Weiterhin liegt eine positive Korrelation moderaten Effektes (vgl. \citep{cohen2013statistical}) mit dem Spearman-Korrelationskoeffizient $r = .378$ zwischen \textbf{\ac{gtn}} und \textbf{\ac{ctn}} vor. Die Spearman-Korrelation ist \textbf{nicht signifikant} ($p = .165 > \alpha = 0.05$) (siehe \textbf{\textit{Abbildung \ref{H2_Korrelation_Auswertung}}}).
...
Während jedem Versuchsdurchlauf besitzt jedes der 3 Teammitglieder die gleiche Avatarkondition. Da die 3 Teammitglieder des Teams die gleiche Anzahl an Runden abgeschlossen haben, besitzen die einzelnen Teammitglieder die gleiche Anzahl an abgeschlossenen Runden. Die Anzahl an abgeschlossenen Runden wird als \ac{tRound} bezeichnet. Die Anzahl der abgeschlossenen Runden jedes einzelnen Teams der Selben Kondition wurde zu jeweils den Variablen: \textit{\ac{tRoundIK}} sowie \textit{\ac{tRoundNIK}} zusammengefasst. Für jedes Team wird zudem ein gemeinsamer kognitiver Vertrauenswert berechnet. Dieser sagt aus, wie viel kognitives Vertrauen das gesamte Team untereinander besitzt und ergibt sich aus der Summe der kognitiven Vertrauenswerte der einzelnen Personen eines Teams. Die kognitiven Vertrauenswerte der Teams der unterschiedlichen Konditionen werden zu den Variablen \textit{\ac{teamCogIK}} sowie \textit{\ac{teamCogNIK}} zusammengefasst. Eine Übersicht dieser Variablen befindet sich in \textbf{\textit{Tabelle \ref{TeamCogTabelle}}})

Im Anschluss wird eine Spearman-Korrelation zwischen \ac{tRound} und \ac{teamCog} auf \textit{Teamebene} durchgeführt, wobei die Teams in verschiedene Konditionen aufgeteilt werden. Damit anschließend überprüft werden kann, ob sich beide Korrelationen voneinander unterscheiden, wird eine Fisher-Z-Transformation für unabhängige Stichproben durchgeführt.\\
...
%
Das \ac{teamCogIK} ist laut Shapiro-Wilk-Test mit $p = .017 < \alpha = 0.05$ nicht normalverteit. \newline
Das \ac{teamCogNIK} ist laut Shapiro-Wilk-Test mit $p = .713 < \alpha = 0.05$ normalverteit. \newline
Die \ac{tRoundIK} ist laut Shapiro-Wilk-Test mit $p = .844 > \alpha = 0.05$ normalverteit. \newline
Die \ac{tRoundNIK} ist laut Shapiro-Wilk-Test mit $p = .453 > \alpha = 0.05$ normalverteit. \\
Das Lowess-Verfahren deutet Augenscheinlich auf keinen linearen Zusammenhang zwischen\newline \ac{teamCogIK} und \ac{tRoundIK} sowie \ac{teamCogNIK} und \ac{tRoundNIK} hin. Daher wird die \textit{Linearitätsannahme nicht angenommen}.
...
Der Spearman-Korrelationskoeffizient zwischen \textbf{\ac{teamCogIK}} und\newline \textbf{\ac{tRoundIK}} beträgt $r = -.205$. Diese Korrelation ist positiv moderaten Effektes (vgl. \citep{cohen2013statistical}) und ist \textbf{nicht signifikant} ($p = .741 > \alpha = 0.05$). \\
Weiterhin ist eine positive Korrelation moderaten Effektes (vgl. \citep{cohen2013statistical}) mit dem Spearman-Korrelationskoeffizient $r = .462$ zwischen \textbf{\ac{teamCogNIK}} und \textbf{\ac{tRoundNIK}} vorhanden. Diese Spearman-Korrelation ist ebenfalls \textbf{nicht signifikant} ($p = .434 > \alpha = 0.05$).

\paragraph{Fisher-Z-Transformation für unabhängige Stichproben}
Obwohl die Stichprobe mit $N=5$ sehr klein ist, wird eine Fisher-Z-Transformation für unabhängige Stichproben durchgeführt um zu überprüfen, ob die beiden Korrelationen sich voneinander unterscheiden.
...
\begin{table}[H]
	\centering\footnotesize\setstretch{1.2}
	\begin{tabularx}{\textwidth}{r | p{2cm} | p{1.5cm} | p{3cm} | p{1.5cm} | p{1.5cm} p{1.5cm} } 
		ID & Individueller-Kognitiver Vertrauenswert & Kondition & Erfolgreich abgeschlossene Runden \newline(\ac{tRoundIK},\newline \ac{tRoundNIK}) & Team ID & Summe \ac{ct} (\ac{teamCogIK}, \ac{teamCogNIK})\\
		\hline 
		...
		30 & $3,57$ & 2 & 9 & & \\	
	\end{tabularx}
		\caption[Individuelles kognitives Vertrauen und erfolgreich abgeschlossenen Runden]{Tabelle der kognitiven Vertrauenswerte und der realen Teameffektivität - Individuell und pro Team zusammengefasst. Kondition 1 definiert die Kondition \ac{ik} und Kondition 2 definiert die Kondition \ac{nik}.}
...
Auf \textit{Konditionsebene} wird analysiert, ob die teilnehmenden Personen aufgrund der unterschiedlichen Avatarkonditionen eine unterschiedliche reale Teameffektivität besitzen.\\
Die reale Teameffektivität (\ac{tRound}) ist laut Shapiro-Wilk-Test ($p = .113 > \alpha = 0.05$) normalverteit.\\
Im folgenden wurde die Varianzgleichheit der realen Teameffektivitätswerte der unterschiedlichen Avatarkonditionen überprüft. Der Levene-Test zeigt keine Varianzgleichheit zwischen \ac{tRoundIK} und \ac{tRoundNIK} ($L = 10,182; p=.003 < \alpha = 0.05$). Damit kann davon ausgegangen werden, dass die Varianzen sich \textbf{signifikant} voneinander unterscheiden. \\
Die reale Teameffektivität (\ac{tRoundIK}) beträgt bei der Kondition \ac{ik} im Durchschnitt $\tilde x = 9,0000$ mit einer Standardabweichung von $\sigma = 3,2732$.\newline 
Die reale Teameffektivität(\ac{tRoundNIK}) beträgt der Kondition \ac{nik} im Durchschnitt $\tilde x = 9,000$ mit einer Standardabweichung von($\sigma = 1,7320$ (siehe \textbf{\textit{Abbildung \ref{H4_Mittelwerte_Auswertung}}})
...
\paragraph{Mann-Whitney-U-Test}
Die Verteilungen der beiden Gruppen \ac{tRoundIK} und \ac{tRoundNIK} unterschieden sich laut Kolmogorov-Smirnov ($p = .181 > \alpha = 0.05$) \textbf{nicht signifikant} von einander. Es gab \textbf{keinen signifikanten Unterschied} der Mittelwerte der realen Teameffektivität zwischen \ac{tRoundIK} $(\tilde x = 9$) und \ac{tRoundNIK} $(\tilde x = 9)$ mit $U = 103,500, Z = -.377, p = .706 > \alpha = 0.05, r = -,0602$. \\

	\begin{figure}[H]
		\begin{footnotesize}
		\centering
			\includegraphics[scale=0.3]{Abbildungen/AuswertungDiagramme/H4_Mittelwerte.png}	
			\caption[Mittelwerte und Standardabweichung der Teameffektivität]{Mittelwerte und Standardabweichung der Teameffektivität. Links die Kondition \ac{ik} und Rechts die Kondition \ac{nik}}
...
Das Lowess-Verfahren deutet Augenscheinlich auf keinen linearen Zusammenhang zwischen\newline \ac{teamGenIK} und \ac{tRoundIK} sowie \ac{teamGenNIK} und \ac{tRoundNIK} hin. Daher wird die \textit{Linearitätsannahme nicht angenommen}.
...
\paragraph{Spearman-Korrelation}
Es ist eine positive Korrelation schwachen Effektes (vgl. \citep{cohen2013statistical}) mit dem Spearman-Korrelationskoeffizient ($r = .200$) zwischen \textbf{\ac{teamGenIK}} und\newline \textbf{\ac{tRoundIK}} vorhanden. Die Spearman-Korrelation ist \textbf{nicht signifikant} ($p = .747 > \alpha = 0.05$).\\
Weiterhin ist eine positive Korrelation moderaten Effektes (vgl. \citep{cohen2013statistical}) mit dem Spearman-Korrelationskoeffizient ($r = .359$) zwischen \textbf{\ac{teamGenNIK} und \ac{tRoundNIK}} vorhanden. Die Spearman-Korrelation ist \textbf{nicht signifikant} ($p = .553 > \alpha = 0.05$).
...
Der \ac{scpn} beträgt bei der Kondition\newline \ac{nik} im Mittel ($\tilde x = 2.9000$) mit einer Standardabweichung von ($\sigma = .21639$). Laut Mann-Whitney-U-Test gab es \textbf{keinen signifikanten Unterschied} der Mittelwerte\newline ($ Z = -.595; p = .552 > \alpha = 0.05$).
	\begin{figure}[H]
		\begin{footnotesize}
		\centering
			\includegraphics[scale=0.3]{Abbildungen/AuswertungDiagramme/BP_SelfCoPresence.png}
			\caption[Boxplot der selbst wahrgenommenen Co-Presence]{Boxplot der selbst wahrgenommenen Co-Presence}
			\label{BP_Presence}
		\end{footnotesize}
	\end{figure}
...
Der \ac{ocpn} beträgt beider Kondition \ac{nik} im Mittel ($\tilde x =3.1333$) mit einer Standardabweichung von ($\sigma =.37108$). Laut Mann-Whitney-U-Test gab es \textbf{keinen signifikanten Unterschied} der Mittelwerte ($ Z = -1.092; p = .275 > \alpha = 0.05$).
	...
	\begin{figure}[H]
		...
		\centering
			...
			\caption[Boxplot der wahrgenommenen Co-Presence der \dq{}anderen\dq{}]{Boxplot der wahrgenommenen Co-Presence der \dq{}anderen\dq{}}
			\label{BP_Presence}
		\end{footnotesize}
	\end{figure}
...
Der \ac{tpn} beträgt bei der Kondition \ac{nik} im Mittel ($\tilde x = 5.3600$) mit einer Standardabweichung von ($\sigma = .7971$). Laut Mann-Whitney-U-Test gab es \textbf{keinen signifikanten Unterschied} der Mittelwerte ($ Z = -.021; p = .983 > \alpha = 0.05$).
	...
	\begin{figure}[H]
		...
		\centering
			...
			\caption[Boxplot der wahrgenommenen Telepresence]{Boxplot der wahrgenommenen Telepresence}
			\label{BP_Presence}
		\end{footnotesize}
	\end{figure}
...
Die \ac{spi} beträgt bei Kondition\newline \ac{ik} im Mittel ($\tilde x = 4.685$) mit einer Standardabweichung von ($\sigma = 1.883$).
Der \ac{spn} beträgt bei der Kondition \ac{nik} im Mittel ($\tilde x = 4.4952$) mit einer Standardabweichung von ($\sigma = 1.54039$). Laut Mann-Whitney-U-Test gab es \textbf{keinen signifikanten Unterschied} der Mittelwerte ($ Z = -.062; p = .950 > \alpha = 0.05$).
	...
	\begin{figure}[H]
		...
		\centering
			...
			\caption[Boxplot der wahrgenommenen Social-Presence]{Boxplot der wahrgenommenen Social-Presence}
			\label{BP_Presence}
		\end{footnotesize}
	\end{figure}

\paragraph{NASA-TLX \dq{}Anstrengungsmaß\dq{}} 
Der \acf{tlx} ist laut\newline Kolmogoroff-Smirnov-Test mit ($p = .337 > \alpha = 0.05$) normalverteit. 
...
Der \ac{tlxn} beträgt bei der Kondition \ac{nik} im Mittel ($\tilde x = 9.033$) mit einer Standardabweichung von ($\sigma = 2.5813$). Der T-Test für unabhängige Stichproben zeigt \textbf{keine Signifikanten Unterschied} der Mittelwerte ($t(28) = -1.116; p = .276 > \alpha = 0.05$).
	...
	\begin{figure}[H]
		...
		\centering
			...
			\caption[Boxplot des Anstrengungsmaßes]{Boxplot des Anstrengungsmaßes}
			\label{BP_Presence}
		\end{footnotesize}
	\end{figure}
...
Die \ac{tcn} beträgt bei der Kondition\newline \ac{nik} im Mittel ($\tilde x = 4.48$) mit einer Standardabweichung von ($\sigma = .47689$). Laut Mann-Whitney-U-Test gab es einen \textbf{signifikanten Unterschied} der Mittelwerte ($ Z = -2.062; p = .039 < \alpha = 0.05$).
	...
	\begin{figure}[H]
		...
		\centering
			...
			\caption[Mittelwerte und Standardabweichung der kognitiven Vertrauenswerte]{Mittelwerte und Standardabweichung der kognitiven Vertrauenswerte. Links die Kondition \ac{ik} und Rechts die Kondition \ac{nik}}
...
Die \ac{ten} beträgt bei der Kondition\newline \ac{nik} im Mittel ($\tilde x = 4.8533$) mit einer Standardabweichung von ($\sigma = .81929$). Laut Mann-Whitney-U-Test gab es einen \textbf{keinen signifikanten Unterschied} der Mittelwerte ($ Z = -1.172; p = .251 > \alpha = 0.05$).
...
Es ist eine positive Korrelation starken Effektes (vgl. \citep{cohen2013statistical}) mit dem Spearman-Korrelationskoeffizient ($r = .729$) zwischen \textbf{\ac{ctn} und \ac{ten}} auf Konditionsebene zu erkennen. Die Spearman-Korrelation ist \textbf{signifikant} ($p = .002 < \alpha = 0.05$). Siehe \textbf{\textit{Abbildung \ref{SubSig1}}}\\
...
\begin{figure}[h]
  ...
  \caption[Spearman-Korrelation \acs{ctn} und \acs{ten}]{Spearman-Korrelationen der kognitiven Vertrauenswerte auf Konditionslevel mit der subjektiv wahrgenommenen Teameffektivität. Links (a) für die Kondition \ac{ik} und Rechts (b) für die Kondition \ac{nik}}
  \label{SubSig1}
\end{figure}

Weiterhin ist eine positive Korrelation starken Effektes (vgl. \citep{cohen2013statistical}) mit dem Spearman-Korrelationskoeffizient ($r = .532$) zwischen \textbf{\ac{ctn} und \ac{tcn}} auf Konditionsebene zu erkennen. Die Spearman-Korrelation ist \textbf{signifikant} ($p = .041 < \alpha = 0.05$). Siehe \textbf{\textit{Abbildung \ref{SubSig2}}}\\
...
\begin{figure}[h]
  ...
  \caption[Spearman-Korrelationen \acs{ctn} und \acs{tcn}]{Spearman-Korrelationen der kognitiven Vertrauenswerte mit der wahrgenommenen Teamkommunikation. Links (a) für die Kondition \ac{ik} und Rechts (b) für die Kondition \ac{nik}}
  \label{SubSig2}
\end{figure}
\clearpage
\newpage

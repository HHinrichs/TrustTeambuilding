...
\paragraph{Avatararten und Analyseebenen}
...
%Da die teilnehmenden Personen als Team arbeiten, sind einige Zusammenhänge auf Individualebene, einige auf Konditionsebene (\ac{ik} und \ac{nik}) und einige auf Teamebene zu betrachten.
...
\textbf{H1$_{1}$} : Es besteht ein Zusammenhang zwischen dem erzielten General-Trust-Score \ac{gt} einer Person und dem erzielten Cognitive-Trust-Score \ac{ct} einer Person.
...
\begin{itemize}
\item \textbf{Hang zum Vertrauen} Der Hang zum Vertrauen bezieht sich in dieser Studie darauf, wie sehr die teilnehmenden Personen  anderen Personen einen Vertrauensvorschuss gewähren\citep[p.30]{mcallister1995affect} (\textit{vgl. Kapitel \nameref{Vertrauen}}).
\item \textbf{Kognitives Vertrauen} Das kognitive Vertrauen bezieht sich auf die \textit{Überzeugung in die Fähigkeiten oder in die Zuverlässigkeit eines anderen}. \citep[p.30]{mcallister1995affect} (\textit{vgl. Kapitel \nameref{Vertrauen}}).
\item \textbf{Team-Effektivität} Die Teameffektivität wird anhand der Anzahl der abgeschlossenen Runden im Team gemessen (\textit{vgl. Kapitel \nameref{Avatare}}).
\item \textbf{Wahrgenommene Teameffektivität} Die Wahrgenommene Teameffektivität kann von unterschiedlichen Avatarkonditionen beeinflusst werden (\textit{vgl. Kapitel \nameref{Teamwork}}).
\item \textbf{Presence} Presence kann durch unterschiedliche Avatarkonditionen beeinflusst werden (\textit{vgl. Kapitel \nameref{Virtual Reality}}).
\item \textbf{Subjektiv wahrgenommene Arbeitsbelastung} Für unterschiedliche Avatarkonditionen kann die \ac{vr}-Erfahrung unterschiedlich anstrengend sein (\textit{vgl. Kapitel \nameref{Avatare}}).
\item \textbf{Team-Kommunikation} Unterschiedliche Avatarkonditionen können einen unterschiedlichen Einfluss auf die Team-Kommunikation besitzen (\textit{vgl. Kapitel \nameref{Avatare}}).
\end{itemize}
...
Die Fragen der Likert-Skalierung wurden als intervallskaliert angenommen, da nur so das subjektive empfinden der einzelnen Testpersonen hinreichend Analysiert werden kann.
...
\begin{table}[H]
	\centering\footnotesize\setstretch{0.9}
...
%H1$_{0}$ : Steigt der General-Trust-Score einer Person (\ac{gt}), steigt der Cognitive-Trust-Score einer Person (\ac{ct}) nicht.
H1$_{0}$ : Es besteht ein Zusammenhang zwischen dem erzielten General-Trust-Score \ac{gt} einer Person und dem erzielten Cognitive-Trust-Score \ac{ct} einer Person.

\paragraph{Allgemeines}
Die der kognitive Vertrauenswert muss für die \textit{Hypothese 1} in \ac{ik} und  \ac{nik} aufgeteilt werden, da der \ac{ct} aufgrund der unterschiedlichen Avatarkonditionen gebildet wurde und somit einen Bias besitzt.
...
H2$_{0}$ : Der erzielte Cognitive-Trust-Score (\ac{ct}) unterscheidet sich bei den Konditionen \ac{ik} und \ac{nik} signifikant voneinander.

\paragraph{Allgemeines}
Es wird analysiert, ob die teilnehmenden Personen aufgrund der unterschiedlichen Avatarkonditionen einen unterschiedliches kognitives Vertrauen bilden. Dazu wird der Mann-Whitney-U-Test durchgeführt. Die Gruppierungsvariablen sind dabei als \ac{ik} und \ac{nik} definiert.\\
...
%Der T-Wert für unabhängige Stichproben ist kleiner Null ($T = -1,495 < 0$), was auf einen Mittelwertsunterschied zwischen \ac{cti} und \ac{ctn} hinweist. Der Mittelwert $\bar(x) = 4,328$ der Kondition \ac{ik} ist kleiner als der Mittelwert $\bar(x) = 4,573$ der Kondition \ac{nik}.
%Der T-Test für unabhängige Stichproben zeigt bei der Differenz des durchschnittlichen \ac{ct} bei der Kondition \ac{ik} ($\bar(x) = 4,328; \sigma = .676$) und \ac{nik} ($\bar(x) = 4,5733; \sigma =.40614$) \textbf{keine Signifikanz} ($t(27) = -1,495; p = .146 > \alpha = 0.05$).
...
Die Verteilungsform der beiden Gruppen \ac{ik} und \ac{nik} unterscheiden sich laut Kolmogorov-Smirnov ($p = .925 > \alpha = 0.05$) \textbf{nicht signifikant} voneinander. Es gab keinen signifikanten Unterschied der Mittelwerte des \ac{ct} zwischen ($\ac{ik} : \tilde x = 4.20$) und ($\ac{nik} : \tilde x = 4.573)$, ($U = 74,000 , Z = -1.615, p = .106 > \alpha = 0.05, r =-.294$).
...
Jeweils 3 Teammitglieder mit gleicher Avatarkondition besitzen die gleiche Teameffektivität, da diese die gleiche Anzahl an Runden abgeschlossen haben. Die Anzahl der abgeschlossenen Runden pro Team und pro Kondition wurde zu jeweils einer Variable für jedes Team zusammengefasst : \ac{tRoundIK} sowie \ac{tRoundNIK}. Für jedes Team wird eine gemeinsamer Kognitiver-Vertrauenswert berechnet. Dieser sagt aus, wie viel kognitives Vertrauen das gesamte Team untereinander besitzt. Dieser kognitive Vertrauenswert ergibt sich aus der Summe der kognitiven Vertrauensangaben der einzelnen Personen eines Teams. Die kognitiven Vertrauenswerte der Teams der unterschiedlichen Konditionen werden zu den Variablen \ac{teamCogIK} sowie \ac{teamCogNIK} für jedes Team zusammengefasst.
Es wird eine Spearman-Korrelation zwischen \ac{tRound} und \ac{teamGen} auf Teamebene durchgeführt, wobei die Teams in verschiedenen Konditionen aufgeteilt werden. Damit anschließend überprüft werden kann, ob sich die beide Korrelationen signifikant voneinander unterscheiden, wird eine Fischer-Z-Transformation für unabhängige Stichproben duchgeführt.\\
...
\begin{table}[H]
	\centering\footnotesize\setstretch{1.2}
	\caption{Kognitive-Vertrauenswerte mit den erfolgreich abgeschlossenen Runden - Individuell und pro Team zusammengefasst. Kondition 1 definiert die Kondition \ac{ik} und Kondition 2 definiert die Kondition \ac{nik}.}
...
H4$_{0}$ : Die Anzahl erfolgreich abgeschlossener Runden \ac{tRound} unterscheidet sich bei den Konditionen \ac{ik} und \ac{nik} signifikant voneinander.

\paragraph{Allgemeines}

Es wird analysiert, ob die teilnehmenden Personen aufgrund der unterschiedlichen Avatarkonditionen eine unterschiedliche Teameffektivität besitzen. Es wird ein T-Test für unabhängige Stichproben durchgeführt. Die Gruppierungsvariablen sind dabei als \ac{ik} und \ac{nik} definiert.\\
Die Anzahl abgeschlossener Runden \ac{tRound} ist laut Shapiro-Wilk-Test ($p = .113 > \alpha = 0.05$) normalverteit.\\
Es wurde auf Varianzgleichheit überprüft. Der Levene-Test zeigt eine keine Varianzgleichheit zwischen \ac{tRoundIK} und \ac{tRoundNIK} ($L = 10,182; p=.003 < \alpha = 0.05$). Damit kann davon ausgegangen werden, dass die Varianzen sich \textbf{signifikant} voneinander unterscheiden.\\
...
\textbf{H5$_{1}$} : Der generelle Vertrauenswert von Teams \ac{teamGen} korreliert mit der Anzahl abgeschlossener Runden von Teams \ac{tRound} und der Kondition \ac{ik} stärker als von Teams mit der Kondition \ac{nik}. \\
%Besitzt nun Korr. A eine signifikanz und Korr B. nicht, weiss ich, dass die Kondition einen Einfluss auf die anz. abg. Runden besitzt. (Trotzdem sollte ich Fischers-Z-Transformation machen.
%Besitzt nun Korr. A eine signifikanz und Korr B. auch, kann ich schauen, ob diese sich signifikant unterscheiden. (Ich MUSS eine Fischer-Z-Transformation machen)
%Besitzt nun Korr. A keine signifikanz und Korr B. auch nicht, hat der generelle Trust Score keine Auswirkung auf die anz. abgeschlossenen Runden und es ist egal welche Kondition ich laut dieser Hypothese nehme. ( Ich müsste keine Fischer-Z-Transformation machen)
Um die Hypothese 5 auszuwerten, wird eine Tabelle (siehe \textbf{Tabelle \ref{TeamGenTabelle}})aufgestellt, die die individuellen generellen Vertrauenswerte der Personen zu einem generellen Vertrauenswert des Teams zusammenfasst. Die durch die Tabelle erstellte Variable der Summe der individuellen generellen Vertrauenswerte wird im weiterem Verlauf \ac{teamGen} genannt.
Es wird eine Spearman-Korrelation zwischen \ac{tRound} und \ac{teamGen} auf Teamebene durchgeführt, wobei die Teams in verschiedenen Konditionen aufgeteilt werden. Damit überprüft werden kann, ob sich die beide Korrelationen signifikant voneinander unterscheiden, wird eine Fischer-Z-Transformation für unabhängige Stichproben duchgeführt.\\
...
\begin{table}[H]
	\centering\footnotesize\setstretch{1.2}
	\caption{Generelle-Vertrauenstabelle mit den erfolgreich abgeschlossenen Runden - Individuell und pro Team zusammengefasst. Kondition 1 definiert die Kondition \ac{ik} und Kondition 2 definiert die Kondition \ac{nik}.}
...
Der \ac{tlxn} beträgt bei der Kondition \ac{nik} im Mittel ($\tilde x = 7,4800$) mit einer Standardabweichung von ($\sigma = 3,10925$). Der T-Test für unabhängige Stichproben zeigt \textbf{keine Signifikante} Differenzt der Mittelwerte ($t(28) = -,612; p = .545 > \alpha = 0.05$).
...
\textbf{H2$_{1}$} : Der erzielte Cognitive-Trust-Score (\ac{ct}) unterscheidet sich bei den Konditionen \ac{ik} und \ac{nik} signifikant voneinander.\\
		...
		\textbf{H3$_{1}$} : Ein hoher Cognitiver-Trust-Score im Team (\ac{teamCogIK} und \ac{teamCogNIK}) hat \textbf{keinen Einfluss} auf die auf die Anzahl der erfolgreich abgeschlossenen Runden (\ac{tRound}) bei unterschiedlichen Avatar Verkörperungen (\ac{tRoundIK} und \ac{tRoundNIK}). \\
		& nicht signifikant \\		
